...
#define XEMBED_EMBEDDED_NOTIFY 1
    ...
    for (i = 0; i < LENGTH(keychords); i++)
      if ((code = XKeysymToKeycode(dpy, keychords[i]->keys[currentkey].keysym)))
        ...
        for (k = 0; k < LENGTH(modifiers); k++)
          XGrabKey(dpy, code, keychords[i]->keys[currentkey].mod | modifiers[k],
...
void movekeyboard_x(const Arg *arg) {
  int ocx, ocy, nx, ny;
  Client *c;
  Monitor *m;
...
void movekeyboard_y(const Arg *arg) {
  int ocx, ocy, nx, ny;
  Client *c;
  Monitor *m;
...
void placedir(const Arg *arg) {
  ...
  if (f && f != s) {
    for (fprior = f->mon->clients; fprior && fprior->next != f;
         fprior = fprior->next)
      ;
    for (sprior = s->mon->clients; sprior && sprior->next != s;
         sprior = sprior->next)
      ;
